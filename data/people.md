# Pasta Lab people

Faculty are `###` blocks with a key/value list. Students are one bullet each,
`Name | Affiliation`, with an optional third field for a note. Wrap the name in
a Markdown link when they have a homepage:

    - Someone | Their University
    - [Someone](https://their.site) | Their University | co-advised with [Name](https://example.com)

Faculty keys: role, affiliation, department, photo, website, scholar, github.
`photo` is the file stem in assets/people/, without the style suffix. Each PI
needs all three styles present: <stem>-chibi-v2.webp, <stem>-lego.webp and
<stem>-minecraft.webp.

This paragraph is ignored by the site.

## Faculty

### Zhuo Zhang

role: Assistant Professor
affiliation: Columbia University
department: Department of Computer Science
photo: zhuo-zhang
website: https://zzhang.xyz/
scholar: https://scholar.google.com/citations?user=9kwvR6sAAAAJ
github: https://github.com/ZhangZhuoSJTU
interests: Security for software systems, binary analysis, AI security, Web3

### Guannan Wei

role: Assistant Professor
affiliation: Tufts University
department: Department of Computer Science
photo: guannan-wei
website: https://continuation.passing.style/
scholar: https://scholar.google.com/citations?user=b_6L2goAAAAJ
github: https://github.com/Kraks
interests: Programming Languages, metaprogramming, program analysis, type systems, compilers

### Yangruibo (Robin) Ding

role: Assistant Professor
affiliation: University of California, Los Angeles
department: Department of Computer Science
photo: robin-ding
website: https://robin-y-ding-columbia.github.io/
scholar: https://scholar.google.com/citations?user=lmgRCxkAAAAJ
interests: Language models and agentic AI for software engineering and security

## Students

- [Dinghong Zhong](https://butter-xz.com/) | Tufts University
- [Jonah Weinbaum](https://jonahweinbaum.github.io/) | Tufts University | co-advised with [Jeff Foster](https://www.cs.tufts.edu/~jfoster/)
- Jun Tan | Research intern at Tufts University
- Pingxuan Li | Research intern at Tufts University 
- Songwen Zhao | University of California, Los Angeles
- [Yimin Wang](https://pinkbubblebubble.github.io/) | University of California, Los Angeles
- Jeffery Tian | Columbia University
- Tijs Bellefroid | Columbia University | co-advised with [Jason Nieh](https://www.cs.columbia.edu/~nieh/)
- Jun Ren | Columbia University
- Jeremy Spence | Research intern at Columbia University
